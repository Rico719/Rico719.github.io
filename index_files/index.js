...
$(function() {
    ...
    $(document).on('click', '#wcc-img-s', function() {
        $(".page-current").append('<div id="wcc-bg" align="center" style="position: absolute; left: 0; top: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.79);; z-index: 10600; visibility: visible; opacity: 1;">' +
                    '<img style="border-radius: 10px; margin-top: 30px; width: 95%;" src="img/qrcode.png">' +
                    '<div>' +
                        '<img id="wcc-off" style="float:right; margin-top: -10px; margin-right: 3%; width: 32px; height: 32px;" src="img/off.png">' +
                        '<img style="float:right; margin-top: -4px; width: 22px; height: 22px;" src="img/big.png">' +
                    '</div>' +
                '</div>');
    });
    ...
    $(document).on('click', '#index-button', function() {
        ...
        window.location.href = 'http://129.211.173.86/finance_proj/html/wcc/testTemp.html?username=' + username;
